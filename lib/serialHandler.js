...
class SerialHandler extends EventEmitter {
    ...
    sendCommand(commandID, targetState) {
        if (this.commandHandler.sendingMessage) {
    ...
    toggleSwitch(switchObject) {
        ...
        try {
            // Send the command
            this.sendCommand(switchType, targetState);
    ...
    async getHeaterTemp(type) {
        ...
        if (type === 'spa') {
            await this.sendCommand('right', null, false);
        } else {
            await this.sendCommand('right', null, false);
    ...
    async setTemperature(type, targetTemp) {
        ...
        if (currentTemp !== null) {
            ...
            for (let i = 0; i < presses; i++) {
                await this.sendCommand(button, null, false);
                await new Promise(resolve => setTimeout(resolve, 1500));
            }
        }
