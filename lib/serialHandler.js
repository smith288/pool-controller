...
class SerialHandler extends EventEmitter {
    constructor(socket_ip, socket_port, mqtt_host = 'localhost', mqtt_port = 1883) {
        ...
        this.socket.on('connect', async () => {
            ...
            try {
                await this.retrieveTemperatures();

                // call retrieveTemperatures every hour
                setInterval(async () => {
                    await this.retrieveTemperatures();
                }, 3600000);
            } catch (error) {
    ...
    async setTemperature(type, targetTemp) {
        ...
        if (currentTemp !== null) {
            if (targetTemp === 0) {
                // For pool temperature, cycle through until we find "Off"
                let foundOff = false;
                let attempts = 0;
                const maxAttempts = 50; // Safety limit
                
                while (!foundOff && attempts < maxAttempts) {
                    const display = this.commandHandler.currentLEDDisplay;
                    if (display && display.display.toLowerCase().includes('off')) {
                        console.log('Found OFF state');
                        foundOff = true;
                    } else {
                        console.log('Not at OFF state, pressing minus...');
                        await this.sendCommand('minus', null, true);
                        await new Promise(resolve => setTimeout(resolve, 1500));
                        attempts++;
                    }
                }
                ...
                if (!foundOff) {
                    throw new Error('Could not find OFF state');
                }
            } else {
    ...
    async getCurrentDisplayTemp() {
        ...
        while (attempts < maxAttempts) {
            ...
            // Check for OFF state first
            if (displayText.match(/(Spa|Pool) Heater1 - Off/)) {
                console.log('Found OFF state, returning 64');
                return 64;
            }
    ...
    async retrieveTemperatures() {
        try {
            ...
            if (diagnostics === null) {
                console.log('Timeout waiting for diagnostic temperatures');
                return;
            }
            ...
            if (spaTemp === null) {
                console.log('Timeout waiting for spa temperature');
                return;
            }
            ...
            if (poolTemp === null) {
                console.log('Timeout waiting for pool temperature');
                return;
            }
